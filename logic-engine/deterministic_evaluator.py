...
def evaluate_lead(*, lead_data: dict, campaign_type: str, target_industry: str, heuristic_flags: dict) -> dict:
    ...
    has_contact_page = any("contact" in f"{x.get('label','')} {x.get('url','')}".lower() for x in anchor_hrefs)
    has_privacy = any("privacy" in f"{x.get('label','')} {x.get('url','')}".lower() for x in anchor_hrefs)
    ...
    has_reviews = any(token in lower_text for token in ["testimonial", "reviews"]) or (lead_data.get("rating_count") or 0) > 5
    ...
    # elementary checking for signals based on campaign type
    missing = []
    if campaign_type == "website_modernization":
        ...
        social_count = sum(1 for x in anchor_hrefs if any(d in x.get("url", "") for d in ["facebook.com", "instagram.com", "linkedin.com", "youtube.com", "pinterest.com", "tiktok.com"]))
        if social_count == 0:
    ...
    return {
        ...
        "overall_digital_health": (
            ...
            if is_qualified_lead else
            "Either not enough business evidence or not enough campaign-relevant gaps."
        ),
